...
Swiper.prototype = {
  init: function() {
    ...
    if (this.imgArr.length > 2) {
      ...
      this.mainDom.style.width = `${(this.imgArr.length + 2) *
        (this.imgWidth + this.gap / 2)}px`
    } else {
      this.nowIndex = 0
    ...
    resImgArr.forEach((item, index) => {
      str += `<a href="${resImgArr[index].url}"><img class="swiper-slide${
        this.clsSuffix
      }" style="width: ${this.imgWidth}px;" src="${
        resImgArr[index].imgPath
      }" /></a>`
    })
  ...
  setScale: function() {
    ...
    if (this.gap < 0) {
      for (let i = 0; i < this.imgDoms.length; i++) {
        if (this.imgArr.length === 2) {
          this.imgDoms[0].style.left = `${this.containerWidth / 4 -
            this.imgWidth / 2}px`
          this.imgDoms[1].style.left = `${(this.containerWidth / 4) * 3 -
            this.imgWidth / 2}px`
        } else if (this.imgArr.length === 1) {
          this.imgDoms[i].style.left = `${this.containerWidth / 2 -
            this.imgWidth / 2}px`
        } else {
          this.imgDoms[i].style.left = `${(i - 1) *
            (this.imgWidth + this.gap)}px`
        }
  ...
  prevSlider: function(aniTime) {
    if (this.imgArr.length === 2) {
      ...
      this.mainDom.style.left = `${parseInt(this.mainDom.style.left) +
        (this.gap + this.imgWidth)}px`
      if (this.nowIndex === 1) {
        this.setScale()
        setTimeout(
          function() {
