...
;(function(window, document, undefined) {
  ...
  L.Draw.Ellipse = L.Draw.SimpleShape.extend({
    ...
    _drawShape: function(latlng) {
      ...
      let radius = [
        this._ylatlng.distanceTo(current_y),
        this._xlatlng.distanceTo(curent_x)
      ]
      if (!this._shape) {
        this._shape = new L.Ellipse(
          this._startLatLng,
          radius,
          0,
    ...
    _fireCreatedEvent: function() {
      var ellipse = new L.Ellipse(
        this._startLatLng,
        [this._shape._mRadiusX, this._shape._mRadiusY],
        0,
        this.options.shapeOptions
      )
    ...
    _onMouseMove: function(e) {
      ...
      if (this._isDrawing) {
        ...
        this._tooltip.updateContent({
          text: this._endLabelText,
          subtext: showRadius
            ? L.drawLocal.draw.handlers.ellipse.radius +
              ': ' +
              L.GeometryUtil.readableDistance(radius[0], useMetric) +
              '  ' +
              L.GeometryUtil.readableDistance(radius[1], useMetric)
            : ''
        })
      }
    }
  ...
  L.Edit.Ellipse = L.Edit.SimpleShape.extend({
    ...
    _createResizeMarker: function() {
      ...
      this._resizeMarkers.push(
        this._createMarker(resizemarkerPointX1, this.options.resizeIcon)
      )
      this._resizeMarkers.push(
        this._createMarker(resizemarkerPointX2, this.options.resizeIcon)
      )
      this._resizeMarkers.push(
        this._createMarker(resizemarkerPointY1, this.options.resizeIcon)
      )
      this._resizeMarkers.push(
        this._createMarker(resizemarkerPointY2, this.options.resizeIcon)
      )
